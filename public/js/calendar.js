								...
								if (layout == 'full') {
									...
									for (var t = 0; t < events.length; t++) {
										if (typeof events[t] != "undefined") {
											...
											var event_name = getShortText(events[t].name, 2)
...
function getEvents(day, month, year) {
	...
	for (var i = 0; i < tiva_events.length; i++) {
		...
		if ((start_date.getTime() <= date_check.getTime()) && (date_check.getTime() <= end_date.getTime())) {
			...
			var event_position = n;
			if (!first_day) {
				event_position = getOrderNumber(tiva_events[i].id, tiva_events[i].day, tiva_events[i].month, tiva_events[i].year);
				var check_events = getEvents(tiva_events[i].day, tiva_events[i].month, tiva_events[i].year);
				for (var j = 0; j < check_events.length; j++) {
					if (check_events[j].id == event.id) {
						if (typeof events[j] == 'undefined' && j != event_position) {
							event_position = j
						}
					}
				}
